...
数据文件位置由服务端配置决定（常见为仓库内 SQLite 文件）。**表结构**以 `packages/server/src/db/schema.ts` 为准；启动时执行 `src/db/migrations/` 下的 SQL 迁移（空库一条 baseline 即建全表）。本地也可用 `pnpm db:push` 快速对齐 schema（不生成迁移文件，适合临时改表）。
...
### 2.2 Project（项目）与 ACL

顶层容器：`project` 表描述项目元数据（标题、描述、状态、优先级等）。**访问控制**由 `project_member` 表实现：每个 Actor（`member` 或 `agent`）在某个项目中的角色为 `owner` | `admin` | `member`。仅 **member** 可创建新项目（创建者自动成为 `owner`）。任务必须属于某个项目：`task.project_id` 在迁移后应非空；对任务及 memory 等子资源的操作均会校验调用者是否为该项目成员。
...
| `project_member` | `(project_id, actor_type, actor_id)` 复合主键，`role` 为 owner/admin/member；级联删除随 `project` |
| `task` | 任务主表；`creator_*` 必填，`assignee_*` 可空；`parent_id` 自引用；`project_id` 指向所属项目 |
| `label` | 标签定义 |
...
`packages/client/src/types/index.ts` 中的 `Task`、`TaskComment`、`Label`、`ActorType`、`Project`、`ProjectMember` 等与 API JSON 形状对齐；协作者列表来自 `GET /api/workspace/actors`，在前端映射为 `WorkspaceMember[]`（`lib/members.ts`），用于筛选器、负责人下拉与**项目成员添加**时的 Actor 选择。当前项目 ID 存于 `uiStore`（`localStorage` 键 `xopc-current-project`），任务列表请求必须带 `projectId` 查询参数。
